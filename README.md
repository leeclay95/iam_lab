...
stored in separate namespaces per account — there is no shared state between
accounts.
...
Floci has a known issue where 12-digit AKIDs break `CreateFunction` and IAM
API calls. The Terraform in this lab uses a dual-provider setup to work around
this:

- S3 and SSM resources are created with `access_key = "111111111111"` so the
  `allowed` profile owns them
- IAM roles and Lambda functions are created with `access_key = "test"` (root)

The Lambda function's env vars include `AWS_ACCESS_KEY_ID=111111111111` so
the runtime reads S3/SSM from the allowed account namespace internally.
...
writes fail silently and all S3/SSM operations return errors.
...
internally so these must exist regardless of shell environment.
...
The lab uses three shells with fixed identities. Open three terminals and
export the following at the start of each session.

**Shell 1 — Allowed** (owns S3 and SSM, runs the test scripts):
```bash
...
To persist across terminal restarts add to `~/.zshrc` or `~/.bashrc`.
Make sure no SSO profile is configured as `[default]` in `~/.aws/config`
as it will intercept calls and cause authentication errors.
...
Lambda pulls its base image on first deploy — allow 1-2 minutes.
...
- `s3:PutObject` as attacker — writes into attacker's own empty namespace,
  not the vault
...
Full reset including all stored Floci data:
